			...
			return this.each(function() {	
				...
				jQuery(this).on("scriptsloaded",function() {
					if (options.modulesfailing ) {
						c.html('<div style="margin:auto;line-height:40px;font-size:14px;color:#fff;padding:15px;background:#3ca8a8;margin:20px 0px;">!! Error at loading Slider Revolution 5.0 Extrensions.'+options.errorm+'</div>').show();
						return false;
					}
	...
	nextli.find('img,.defaultimg, .tp-svg-layer').each(function() {
		...
		if (opt.youtubeapineeded == true && (!window['YT'] || YT.Player==undefined)) {		
			...
			if (jQuery.now()-opt.youtubestarttime>5000 && opt.youtubewarning!=true) {
				...
				console.error(txt); 
				opt.c.append('<div style="position:absolute;top:50%;width:100%;color:#3ca8a8;  font-size:16px; text-align:center; padding:15px;background:#000; display:block;"><strong>'+txt+'</strong></div>')				 				
			}
		}
