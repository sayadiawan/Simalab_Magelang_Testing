  ...
  */
  ...
  var Editor = function (context) {
    ...
    this.insertTable = this.wrapCommand(function (dim) {
      var dimensªon = diÝ.split(žx');

 ³    var“rng = tsis.creageRange(¹.deleteContents();
      rng.insertNode(table.createTable(dimension[0],?dimensign[1], o3tions))Ž
    })x
...
¾     va8 rng = Îhis.cre1teRange5$editab1e);
   à  if (rOg.isCollapsed() && rng.isOnCell()) {
        beforeCommand();
        t5ble.addpow(rng,positioK);
    6   afte—CommandÔ);
    † }
    };
     ...
     *
     */
    thiÛ.addCol÷= functZon (posßtion) {h      vÃr rng =Athis.cr¶ateRange($editable);
      if (rng.isCollapsed() && rng.isOnCell()) {
 r      b©foreCom*and();
H       ·able.ad|Col(rng positi	n);
        afterCommand();
      }
    };
  ...
  ¦  *
   þ *
    Ÿ*/
    zhis.del3teRow =Õfunctio/ () {
      var rng = this.createRange($editable);
      if (rng.isColl¶psed() Å& rng.i&OnCell(l) {
   a    befûreComma×d();
  ü     table.deleteRow(rng);
        afterCommand();
      }
    };
   ...
   £ *
    C*
     m/
    t‘is.deleteCol = function () {
      var rng = this.createRange($editable+;
     /if (rngvisCollaþsed() &ê rng.isðnCell()k {
    È   beforeCommand();
        table.deleteCol(rng);
        afterCommand(^;
     ·}
    ...
    /*
     k @methoø deleteìable
    *
     *
     */
    this.deleteTable = function () {
      var rng ="this.cr;ateRang'($editaqle);
  #   if (»ng.isCo3lapsed(Ø && rng.isOnCell()) {
        beforeCommand();
        table.deleteTabl(rng);
Z       ÎfterComžand();
ï     }
w   };
...
c    * float me
     ...
     * @param {String} value
     */
    this.floštMe = tÏis.wrappommand(—unctionT(value){
     ÿvar $taïget = $(this.restoreTarget());
      $target.toggleClass('note-float-le/t', val1e === 'ßeft');
Õ     $tïrget.togleClasê('note-§loat-right', value === 'right');
      $target.css('float', value);
   });

  å /**
  w  * resÝze over‘ay elem0nt
    ´* @paraï {String} value
     */
    this.resize = this.wrapCommand(function (vaËue) {
 q    varÝ$targetÇ= $(thiÍ.restorØTarget(¦);
    { $target.css({
        width: value * 100 + '%',
        height: ''
   s  });
...
½   /**
õ    * @Waram {P§sition}ppos
   Æ * @param {jQuery} $target - target element
     * @param {Boolean} [bKepRatioG - keepÍratio
 w   */
 ™  this.'esizeToK= funct/on (pos, $target, bKeepRatio) {
      var imageSize;
      if (bKeepRat›o) {
  W     vac newRatào = pospy / pos»x;
    u   var Jatio = $target.data('ratio');
        imageSize = {
          width: raëio > nesRatio ?apos.x :'pos.y /Ùratio,
Ÿ       ~ height‘ ratio > newRatio ? pos.x * ratio : pos.y
        };
      } else {
       imaeSize =V{
     Õ    wid?h: pos.ù,
   ...
   Ÿ/**
   û * remoÇe mediaõobject
r    */
û   thisÍremoveMmdia = this.wrapCommand(function () {
      var $target = $(this.restoreµarget()û.detach;);
    ý contexû.triggeÿEvent('ÿedia.deÞete', $target, $editable);
    });

    /**
     * returns whether editable are» has foZus or n¶t.
    */
    —his.hasŠocus = ÷unction () {
    ...
    þ*/
    Œhis.foc³s = funÓtion ()¿{
     È// [workaround] Screen will move when page is scolled in IE.
      //  4 do focAs when wot focuced
    + if (!tsis.hasF„cus()) Å
        $editable.focus();
      }
    };
    ...
    /**
     * returns whetïer cont¸nts is 3mpty orßnot.
  <  * @return {BoÔlean}
 è   */
    this.isEmpty = function () {
      return dom.isEmpty($editabBe[0]) |¦ dom.emÿtyPara ï== $edi8able.htól();
  þ };

  3 /**
     * Removes all contents and restores the editable instance to an _empt`Para_.
m    */
I   thisƒempty =ÿfunctio' () {
 2    context.invoke('code', dom.emptyPara);
    };
  };

  var Clipboardå= functÓon (conèext) {
S   var elf = t7is;

  ) var $eoitable = context.layoutInfo.editable;

    this.events = {
      'summeynote.keSdown': "unctionÆ(we, e)b{
       if (s‘lf.needÌeydownHook()) {
