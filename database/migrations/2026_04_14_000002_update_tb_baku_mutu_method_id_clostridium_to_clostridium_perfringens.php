...
/**
 * Pindahkan tb_baku_mutu.method_id dari metode "Clostridium"
 * ke metode "Clostridium perfringens".
 */
class UpdateTbBakuMutuMethodIdClostridiumToClostridiumPerfringens extends Migration
{
    ...
    public function up()
    {
        ...
        $oldIds = DB::table('ms_method')
            ...
            ->whereRaw($expr . ' LIKE ?', ['%clostridium%'])
            ->whereRaw($expr . ' NOT LIKE ?', ['%perfringens%'])
            ->pluck('id_method');
        ...
        $newRow = DB::table('ms_method')
            ->whereNull('deleted_at')
            ->whereRaw($expr . ' LIKE ?', ['%clostridium%'])
            ->whereRaw($expr . ' LIKE ?', ['%perfringens%'])
