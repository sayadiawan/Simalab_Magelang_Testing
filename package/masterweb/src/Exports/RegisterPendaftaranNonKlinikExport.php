...
class RegisterPendaftaranNonKlinikExport implements WithMultipleSheets
{
  /** @var int */
  protected $year;
  ...
  public function sheets(): array
  {
    if ($this->month !== null && $this->month >= 1 && $this->month <= 12) {
      return [
        new RegisterPendaftaranNonKlinikMonthSheet($this->year, $this->month, $this->laboratoriumId, $this->unitLabel, $this->date),
      ];
    }

    $sheets = [];
    for ($m = 1; $m <= 12; $m++) {
      $sheets[] = new RegisterPendaftaranNonKlinikMonthSheet($this->year, $m, $this->laboratoriumId, $this->unitLabel);
    }
