                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-custom bg-inverse-primary">
                            ...
                            <li class="breadcrumb-item"><a href="{{url('/menu')}}"> Master Public Menu</a></li>
      ...
      <div class="card-body">
        ...
        <div class="main-menu-container">
          <div class="card rounded border mb-2 main-menu" id="{{$menu->id}}">
                    ...
                    <div class="d-flex bd-highlight">
                      <div class="mr-auto p-2 bd-highlight">
                        ...
                        <p class="mb-0 text-muted">
                          {{($menu->type == 0)?"link" : "url"}} : {{$menu->link}} | Type : {{$menu->typeMenu->name}}
                        </p>
                      </div>
                      <div class="p-2 bd-highlight">
                        <p>
                          <form onsubmit="return confirm('Delete this menu permanently?')" class="d-inline" action="{{url('menu/destroy/'.$menu->id)}}" method="POST">
                            @csrf
            ...
            @php
                $subdata = \Smt\Masterweb\Models\Menu::all()->where('publish','1')->sortBy('order')->where('upmenu',$menu->id);
            @endphp
                      ...
                      <div class="media-body">
                          <div class="d-flex bd-highlight">
                            <div class="mr-auto p-2 bd-highlight">
                              ...
                              <p class="mb-0 text-muted">
                                {{($submenu->type == 0)?"link" : "url"}} : {{$submenu->link}} | Type : {{$submenu->typeMenu->name}}
                              </p>
                            </div>
                            <div class="p-2 bd-highlight">
                              <p>
                                <form onsubmit="return confirm('Delete this submenu permanently?')" class="d-inline" action="{{url('menu/destroy/'.$menu->id)}}" method="POST">
                                  @csrf
                                  <input type="hidden" name="_method" value="DELETE">
                                  <button type="button" class="btn btn-sm btn-primary edit-module" data-id="{{$submenu->id}}">Edit</button>
                                  <button class="btn btn-sm btn-danger">Delete</button>
        ...
        <div class="modal-body">
          <form method="post" action="{{url('menu/store')}}">
              ...
              <select name="type" class="form-control">
                ...
                <input type="text" id="typeNew" class="form-control" name="typeNew" value="" placeholder="atau buat type baru">  
              ...
              <label>Type</label>
              <input type="radio" name="type_link" value="0" checked>Link | <input type="radio" name="type" value="1">Url
        </div>
