        ...
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">
                        <i class="fas fa-user-plus mr-2"></i>Petugas Management
                    </h4>
                </div>
                <div class="card-body">
                    <p class="text-muted mb-4">
                        <i class="fas fa-info-circle mr-1"></i>Tambah data petugas baru
                    </p>
                    <form enctype="multipart/form-data" class="forms-sample" action="{{ route('adm-petugas-store') }}"
                        method="POST">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="gelar" name="gelar"
                                        placeholder="Masukkan gelar" value="{{ old('gelar') }}">
                                    @error('gelar')
                                        <div class="error text-danger mt-1 text-small">
                                            <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="password" name="password"
                                        placeholder="Password BSRE dinonaktifkan sementara" value="{{ old('password') }}"
                                        disabled>
                                    @error('password')
                                        <div class="error text-danger mt-1 text-small">
                                            <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2-multiple" id="lab_id" name="lab_id[]"
                                        ...
                                        <option value="NON_LAB"
                                            {{ is_array(old('lab_id')) && in_array('NON_LAB', old('lab_id')) ? 'selected' : '' }}>
                                            Non Lab (Pendaftaran/Kepala Lab/Keuangan)
                                        </option>
                                        @foreach ($laboratoriums as $lab)
                                            <option value="{{ $lab->id_laboratorium }}"
                                                data-lab-code="{{ $lab->kode_laboratorium }}"
                                                {{ is_array(old('lab_id')) && in_array($lab->id_laboratorium, old('lab_id')) ? 'selected' : '' }}>
                        ...
                        <div class="form-group" id="role-per-lab-group" style="display: none;">
                            <label>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateRolePerLab() {
                ...
                // Get old role_per_lab data
                var oldRolePerLab = @json(old('role_per_lab', []));
                ...
                $('#lab_id option:selected').each(function() {
                    ...
                    if (labId === 'NON_LAB') {
                        ...
                        loadRolesForLab('NON_LAB', '#role_per_lab_NON_LAB', oldRolePerLab['NON_LAB'] || []);
                    } else {
                        ...
                        loadRolesForLab(labId, '#role_per_lab_' + labId, oldRolePerLab[labId] || []);
                    }
                });
            }
            ...
            if ($('#lab_id').val() && $('#lab_id').val().length > 0) {
                updateRolePerLab();
            }
            ...
            $('.form-group').each(function(index) {
                $(this).css('animation', 'fadeInUp 0.5s ease ' + (index * 0.1) + 's both');
            });
        });
    </script>
@endsection
