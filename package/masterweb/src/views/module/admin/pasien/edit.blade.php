            ...
            <form enctype="multipart/form-data" class="forms-sample" id="form"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group" style="position: relative;">
                                        ...
                                        <input type="text" class="form-control" name="nik_pasien" id="nik_pasien"
                                            ...
                                            value="{{ $item->nik_pasien ?? old('nik_pasien') }}" maxlength="16"
                                    ...
                                    <div id="satu_sehat_status" class="mt-2"
                                        style="{{ $item->id_pasien_satu_sehat ? 'display: block;' : 'display: none;' }}">
                                        <small class="badge badge-success" id="satu_sehat_badge">
                                            <i class="fa fa-check-circle"></i> Data ditemukan di Satu Sehat
                                            @if ($item->id_pasien_satu_sehat)
                                                (ID: {{ $item->id_pasien_satu_sehat }})
                                            @endif
                                        </small>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" name="nama_pasien" id="nama_pasien"
                                            ...
                                            value="{{ mb_strtoupper((string) old('nama_pasien', $item->nama_pasien ?? ''), 'UTF-8') }}"
                                        ...
                                        <div class="card-body p-3"
                                            ...
                                            <div class="form-check d-flex align-items-center">
                                                <input type="radio" class="form-check-input" name="gender_pasien"
                                                    id="gender_pasien_male" value="L"
                                                    {{ ($item->gender_pasien ?? 'L') == 'L' ? 'checked' : '' }}
                                        ...
                                        <div class="card-body p-3"
                                            ...
                                            <div class="form-check d-flex align-items-center">
                                                <input type="radio" class="form-check-input" name="gender_pasien"
                                                    ...
                                                    {{ ($item->gender_pasien ?? 'L') == 'P' ? 'checked' : '' }}
                                                    style="cursor: pointer; width: 20px; height: 20px;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="tmpt_lahir" id="tmpt_lahir"
                                    ...
                                    value="{{ old('tmpt_lahir', $item->tmpt_lahir ?? '') }}"
                                    style="border: 2px solid #e2e8f0; border-left: none; font-size: 15px; height: 45px;">
                            </div>
                            <small class="form-text text-muted">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="phone_pasien" id="phone_pasien"
                                    ...
                                    value="{{ $item->phone_pasien ?? old('phone_pasien') }}"
                                    style="border: 2px solid #e2e8f0; border-left: none; font-size: 15px; height: 45px;">
                            </div>
                            <small class="form-text text-muted">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="alamat_pasien" id="alamat_pasien" rows="3"
                        ...
                        style="border: 2px solid #e2e8f0; border-radius: 8px;">{{ $item->alamat_pasien ?? old('alamat_pasien') }}</textarea>
                    <small class="form-text text-muted">
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Set existing birth date if available
            @if ($item->tgllahir_pasien)
                const existingDate =
                    '{{ \Carbon\Carbon::createFromFormat('Y-m-d', $item->tgllahir_pasien)->format('d/m/Y') }}';
                const dateParts = existingDate.split('/');
                if (dateParts.length === 3) {
                    $('#birth_day').val(dateParts[0]);
                    $('#birth_month').val(dateParts[1]);
                    $('#birth_year').val(dateParts[2]);
                    $('#tgllahir_pasien').val(existingDate);
                    updateBirthDate();
                }
            @endif
            ...
            // NIK input validation and Satu Sehat integration
            $('#nik_pasien').on('input', function() {
                this.value = this.value.replace(/[^\d]/g, '');
                const nik = $(this).val();
                const nama = $('#nama_pasien').val();

                if (nik.length === 16 && nama && nama.length >= 3) {
                    // Fetch Satu Sehat data
                    $.ajax({
                        url: "{{ route('elits-pasien.index') }}",
                        type: 'GET',
                        data: {
                            check_satu_sehat: true,
                            nik: nik,
                            nama: nama
                        },
                        success: function(response) {
                            if (response.id_satu_sehat) {
                                $('#satu_sehat_status').show();
                                $('#satu_sehat_badge').html(
                                    `<i class="fa fa-check-circle"></i> Data ditemukan di Satu Sehat (ID: ${response.id_satu_sehat})`
                                );
                            }
                        }
                    });
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Set existing wilayah if available
                    @if (isset($wilayahParents) && $wilayahParents)
                        @if (isset($wilayahParents['provinsi_id']))
                            $('#provinsi_pasien').val('{{ $wilayahParents['provinsi_id'] }}').trigger(
                                'change');
                            setTimeout(function() {
                                @if (isset($wilayahParents['kabupaten_id']))
                                    $('#kabupaten_pasien').val(
                                        '{{ $wilayahParents['kabupaten_id'] }}').trigger(
                                        'change');
                                    setTimeout(function() {
                                        @if (isset($wilayahParents['kecamatan_id']))
                                            $('#kecamatan_pasien').val(
                                                '{{ $wilayahParents['kecamatan_id'] }}'
                                                ).trigger('change');
                                            setTimeout(function() {
                                                @if (isset($wilayahParents['desa_id']))
                                                    $('#desa_pasien').val(
                                                        '{{ $wilayahParents['desa_id'] }}'
                                                        );
                                                @endif
                                            }, 500);
                                        @endif
