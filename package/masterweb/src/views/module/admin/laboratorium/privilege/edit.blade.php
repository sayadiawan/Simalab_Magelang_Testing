...
@section('content')
    <div class="card">
        ...
        <form enctype="multipart/form-data" class="forms-sample" action="{{route('privileges-elits.update',[$privilege->id])}}" method="POST">
            @csrf
            ...
            @endif
            <br>
            @if (!isset($privilegemenuroleuser))
                    ...
                    <table class="table table-bordered table-stripped">
                        ...
                        @foreach ($privilegemenurole as $privilegemenurole)
                            @php
                                ...
                                $value = $privilegemenurole->value;
                            @endphp
                            <tr>
                                <td align="center">{{$no}}</td>
                                <td>{{$name}}</td>
                                <td>
                                    <input type="checkbox" name="{{join('-',explode(' ', $name))}}[{{$privilegemenurole->id_privilege_features}}]" {{($value == "1") ? "checked" : ""}} data-toggle="toggle">
                                </td>
                            </tr>
                    ...
                    <table class="table table-bordered table-stripped">
                        ...
                        @foreach ($privilegemenurole as $privilegemenurole)
                            @php
                                $ketemu=false;
                            @endphp
                            @foreach ($privilegemenuroleuser as $myprivilegemenuroleuser)
                                @if ($myprivilegemenuroleuser->id_privilege_features==$privilegemenurole->id_privilege_features)
                                    @php
                                        $no++;
                                        $name = $privilegemenurole->name_privilege_features;
                                        $value = $myprivilegemenuroleuser->value;
                                        $ketemu = true;
                                    @endphp
                                    <tr>
                                        <td align="center">{{$no}}</td>
                            ...
                            <tr>
                                ...
                                <td>{{$name}}</td>
                                <td>
                                    <input type="checkbox" name="{{join('-',explode(' ', $name))}}[{{$privilegemenurole->id_privilege_features}}]" data-toggle="toggle">
                                </td>
                            </tr>
                            @endif
                        @endforeach
