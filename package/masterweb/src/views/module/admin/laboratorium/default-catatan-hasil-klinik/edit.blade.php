...
@section('title')
    Edit Default Catatan Hasil Klinik
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form id="form-default-catatan" class="forms-sample" action="{{ route('elits-default-catatan-hasil-klinik.update', [$id]) }}" method="POST" novalidate>
                @csrf
                    ...
                    <select name="parameter_satuan_klinik" id="parameter_satuan_klinik" class="form-control select2">
                        ...
                        @foreach ($parameters as $param)
                            <option value="{{ $param->id_parameter_satuan_klinik }}"
                                {{ old('parameter_satuan_klinik', $item->parameter_satuan_klinik) == $param->id_parameter_satuan_klinik ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <textarea name="catatan_default" id="catatan_default" class="form-control" rows="8">{{ old('catatan_default', $item->catatan_default) }}</textarea>
                    <small class="form-text text-muted">Perubahan di sini akan menjadi default baru di form analis (untuk catatan yang masih mengikuti master).</small>
                </div>
