...
@section('content')
                ...
                <div class="card-body" style="padding: 15px 20px;">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ url('/elits-samples', [$sample->permohonanuji->id_permohonan_uji]) }}">Data
                                    Sampel</a>
                            </li>
    ...
    <!-- Form -->
    <form action="{{ route('elits-samples.update', [$sample->id_samples]) }}" method="POST" id="form-create-sample"
        enctype="multipart/form-data">
        @csrf
        {{-- Backend menyinkronkan jenis/paket/parameter hanya jika changeSampleType=1 (di-set JS saat user mengubahnya). Edit nomor sampel saja tidak mengaktifkan flag ini. --}}
        <input type="hidden" name="changeSampleType" id="changeSampleType" value="0">
        ...
        <div class="form-wizard-container">
            ...
            <div class="form-step active" data-step="1">
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item">
                            @php
                                ...
                                if (isset($sample)) {
                                    if (!empty($sample->datesampling_samples)) {
                                        try {
                                            $fpDatesampling = \Carbon\Carbon::parse($sample->datesampling_samples)->format('d/m/Y H:i');
                                        } catch (\Throwable $e) {
                                        }
                                    }
                                    if (!empty($sample->date_sending)) {
                                        try {
                                            $dtSend = \Carbon\Carbon::parse($sample->date_sending);
                                            $fpDateSending = $dtSend->format('d/m/Y H:i');
                                            $fpDateSendingStop = $dtSend->copy()->addMinutes(10)->format('d/m/Y H:i');
                                        } catch (\Throwable $e) {
                                        }
                                    }
                                }
                ...
                <div class="form-section-card-sample mt-4">
                    ...
                    <div class="col-lg-12">
                        <div class="form-group">
                            <label for="cost_sampling" style="font-size: 14px; color: #4a5568;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" name="cost_sampling" id="cost_sampling"
                                    value="{{ old('cost_sampling') ?? ($sample->cost_sampling_samples ?? 20000) }}" placeholder="20000"
                ...
                <div class="col-lg-12 mt-2 mb-4">
                    ...
                    <div class="form-section-card-sample">
                        ...
                        <div class="is_paket" style="display: none">
                            <input type="hidden" id="is_paket" name="is_paket"
                                value="{{ !empty($selected_packet_id) ? 'true' : 'false' }}">
                        </div>
                                                                                        ...
                                                                                        <div class="form-group">
                                                                                            <div class="form-check">
                                                                                                <input name="method[]"
                                                                                                    ...
                                                                                                    data-prices-by-sample-type="{{ e(json_encode($method->prices_by_sample_type ?? [])) }}"
        ...
        function initializeSampleFlatpickr() {
            ...
            waitForFlatpickr(function() {
                ...
                flatpickr('#datesampling_samples', Object.assign({}, fpBase, {
                    defaultDate: parseDefaultDate(defaults.datesampling) || new Date()
                }));
                ...
                flatpickr('#date_sending', Object.assign({}, fpBase, {
                    defaultDate: parseDefaultDate(defaults.dateSending) || new Date(),
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(function() {
                ...
                if (stepIndicator && formSteps.length > 0) {
                    ...
                    // Mark wizard as initialized (to enable scroll on navigation)
                    setTimeout(function() {
                        window.wizardInitialized = true;
                        console.log('✓ Wizard ready for navigation');
                    }, 1000);
                } else {
                ...
                console.log('Step 2: Pre-populating existing data...');
                window.__editSampleBootstrapping = true;

                // Jangan matikan bootstrap sebelum jenis+paket+method selesai,
                // kalau tidak changeSampleType ikut jadi 1 dan edit nomor sampel saja gagal.
                window.finishEditSampleBootstrap = function() {
                    window.__editSampleBootstrapping = false;
                    var changeEl = document.getElementById('changeSampleType');
                    if (changeEl) {
                        changeEl.value = '0';
                    }
                    console.log('✓ Edit sample bootstrap finished (changeSampleType reset to 0)');
                };

                // Debug: Show sample data
                console.log('=== EDIT MODE DEBUG ===');
                console.log('Sample ID:', '{{ $sample->id_samples ?? 'N/A' }}');
                console.log('Sample Type ID:', '{{ $sample->typesample_samples ?? 'N/A' }}');
                console.log('Packet ID (pac):', '{{ $sample->pac ?? 'N/A' }}');
                console.log('Selected Packet ID:', '{{ $selected_packet_id ?? 'N/A' }}');

                @if (isset($sample) && isset($sample->typesample_samples))
                    // Pre-select jenis sampel
                    const selectedSampleTypeId = '{{ $sample->typesample_samples }}';
                    console.log('Pre-selecting sample type:', selectedSampleTypeId);

                    setTimeout(function() {
                        const sampleTypeButton = document.querySelector(
                            `.btn-pick-jenis[data-id="${selectedSampleTypeId}"]`);
                        if (sampleTypeButton) {
                            sampleTypeButton.click();
                            console.log('✓ Sample type pre-selected');

                            @if (isset($selected_packet_id) && $selected_packet_id)
                                // Pre-select paket after jenis sampel is selected
                                // Need longer delay because packet buttons are loaded dynamically
                                setTimeout(function() {
                                    const selectedPacketId = '{{ $selected_packet_id }}';
                                    console.log('Pre-selecting packet:', selectedPacketId);

                                    // Function to try selecting packet with retry
                                    function trySelectPacket(attempt = 1, maxAttempts = 5) {
                                        console.log(
                                            `Attempt ${attempt} to select packet...`);

                                        // Debug: Show all available packet buttons
                                        const allPacketButtons = document.querySelectorAll(
                                            '.btn-pick-paket');
                                        console.log('Total packet buttons found:',
                                            allPacketButtons.length);
                                        allPacketButtons.forEach((btn, idx) => {
                                            console.log(`  Packet ${idx + 1}:`, {
                                                text: btn.textContent
                                                    .trim(),
                                                'data-id': btn.getAttribute(
                                                    'data-id'),
                                                'data-idpacket': btn
                                                    .getAttribute(
                                                        'data-idpacket')
                                        ...
                                        let packetButton = document.querySelector(
                                            `.btn-pick-paket[data-id="${selectedPacketId}"]`
                                        );

                                        if (!packetButton) {
                                            packetButton = document.querySelector(
                                                `.btn-pick-paket[data-idpacket="${selectedPacketId}"]`
                                            );
                                        }

                                        console.log('Packet button found:', packetButton !==
                                            null);
                                        if (packetButton) {
                                            console.log('Found packet button:', {
                                                text: packetButton.textContent
                                                    .trim(),
                                                'data-id': packetButton
                                        ...
                                        if (packetButton) {
                                            packetButton.click();
                                            console.log('✓ Packet pre-selected via button');
                                            // Method pre-check biasanya sudah/akan jalan terpisah;
                                            // tunggu sebentar lalu tutup bootstrap.
                                            setTimeout(window.finishEditSampleBootstrap, 800);
                                        } else if (attempt < maxAttempts) {
                                            console.log(
                                                `⚠️ Packet button not found, retrying in 500ms...`
                                            );
                                            setTimeout(() => trySelectPacket(attempt + 1,
                                                maxAttempts), 500);
                                        } else {
                                            console.log(
                                                '⚠️ Max attempts reached, trying dropdown...'
                                            );
                                            const packetSelect = document.getElementById(
                                                'packet');
                                            if (packetSelect) {
                                                const option = packetSelect.querySelector(
                                                    `option[value="${selectedPacketId}"]`
                                                );
                                                console.log('Packet option found:',
                                                    option !== null);
                                                if (option) {
                                                    option.selected = true;
                                                    $(packetSelect).trigger('change');
                                                    console.log(
                                                        '✓ Packet pre-selected via dropdown'
                                                    );
                                                } else {
                                                    console.log(
                                                        '✗ Packet option not found');
                                                }
                                            } else {
                                                console.log('✗ Packet select not found');
                                            }
                                            setTimeout(window.finishEditSampleBootstrap, 300);
                                        }
                                    }
                            ...
                            @else
                                setTimeout(window.finishEditSampleBootstrap, 700);
                            @endif
                        } else {
                            window.finishEditSampleBootstrap();
                        }
                    }, 200);
                @endif

                @if (isset($methods) && count($methods) > 0)
                    // Pre-check selected parameters
                    const selectedMethodIds = [
                        @foreach ($methods as $method)
                            {
                                method_id: '{{ $method->method_id }}',
                                lab_id: '{{ $method->laboratorium_id }}'
                            },
                        @endforeach
                    ];
                    console.log('Pre-selecting methods:', selectedMethodIds);

                    setTimeout(function() {
                        selectedMethodIds.forEach(function(item) {
                            const checkbox = document.querySelector(
                                `.checkbox[data-idmethod="${item.method_id}"][data-idlabs="${item.lab_id}"]`
                            );
                            if (checkbox && !checkbox.checked) {
                                checkbox.checked = true;
                                window.__suppressChangeSampleType = true;
                                // Trigger change event to update cart
                                $(checkbox).trigger('change');
                                window.__suppressChangeSampleType = false;
                                console.log('✓ Method pre-checked:', item.method_id);
                            }
                        });

                        // Update cart widget after pre-selecting
                        ...
                        if (typeof window.moveCheckedParametersToTop === 'function') {
                            window.moveCheckedParametersToTop();
                        }

                        console.log('✓ All methods pre-selected and cart updated');
                        @if (!isset($selected_packet_id) || !$selected_packet_id)
                            window.finishEditSampleBootstrap();
                        @endif
                    }, 500);
                ...
                @if (isset($has_input_hasil) && $has_input_hasil)
                    // Disable form if sample already has input hasil
                    console.log('⚠️ Sample has input hasil - disabling form fields...');

                    setTimeout(function() {
                        // Disable jenis sampel buttons
                        document.querySelectorAll('.btn-pick-jenis').forEach(function(btn) {
                            btn.disabled = true;
                        ...
                        // Disable paket buttons
                        document.querySelectorAll('.btn-pick-paket').forEach(function(btn) {
                            btn.disabled = true;
                            btn.style.opacity = '0.6';
                            btn.style.cursor = 'not-allowed';
                            btn.style.pointerEvents = 'none';
                        ...
                        // Disable parameter checkboxes
                        document.querySelectorAll('.checkbox[name="method[]"]').forEach(function(
                            checkbox) {
                        ...
                        // Disable packet select
                        const packetSelect = document.getElementById('packet');
                        if (packetSelect) {
                            packetSelect.disabled = true;
                            packetSelect.style.opacity = '0.6';
                        }

                        // Add warning message at the top of form
                        const formContainer = document.querySelector('.form-wizard-container');
                        if (formContainer && !document.querySelector('.input-hasil-warning')) {
                            const warningDiv = document.createElement('div');
                            warningDiv.className = 'alert alert-warning input-hasil-warning';
                            warningDiv.style.marginBottom = '20px';
                            warningDiv.style.borderLeft = '5px solid #ff9800';
                            warningDiv.innerHTML =
                                '<i class="fa fa-exclamation-triangle"></i> <strong>Perhatian:</strong> Sampel ini sudah memasuki tahap <strong>Input Hasil</strong>. Jenis sampel, paket, dan parameter tidak dapat diubah.';
                            formContainer.parentNode.insertBefore(warningDiv, formContainer);
                        }

                        console.log('✓ Form fields disabled due to existing input hasil');
                    }, 800);
                @endif

                // Then setup validation
                console.log('Step 3: Setting up validation...');
                validateForm();
            ...
            function markSampleTypeOrParameterChanged() {
                if (window.__editSampleBootstrapping || window.__suppressChangeSampleType) {
                    return;
                }
                var el = document.getElementById('changeSampleType');
                if (el) {
                    el.value = '1';
                }
            }
            ...
            document.addEventListener('click', function(e) {
                if (e.target.classList.contains('btn-pick-jenis') ||
                    ...
                    markSampleTypeOrParameterChanged();
            ...
            $(document).on('change', '.checkbox[name="method[]"]', function() {
                markSampleTypeOrParameterChanged();
            });

            // Safety: pastikan flag bootstrap tidak menahan changeSampleType selamanya
            setTimeout(function() {
                if (!window.__editSampleBootstrapping) {
                    return;
                }
                if (typeof window.finishEditSampleBootstrap === 'function') {
                    window.finishEditSampleBootstrap();
                } else {
                    window.__editSampleBootstrapping = false;
                    var changeEl = document.getElementById('changeSampleType');
        ...
        $(document).ready(function() {
            ...
            window.applyMethodPricesForJenisSampel = function(sampletypeId) {
                ...
                $('.method-row .checkbox').each(function() {
                    ...
                    var resolved = def;
                    if (sid && prices && prices[sid] !== undefined && prices[sid] !== null && prices[sid] !== '') {
                        var p = parseFloat(prices[sid]);
                        if (!isNaN(p)) {
                            resolved = p;
                        }
                    }
            ...
            function filterParametersByJenisSampel(sampletypeId) {
                ...
                if (sampletypeId && typeof window.applyMethodPricesForJenisSampel === 'function') {
                    window.applyMethodPricesForJenisSampel(sampletypeId);
                }
            }
        ...
        $(document).ready(function() {
            ...
            $("#form-create-sample").validate({
                ...
                submitHandler: function(form) {
                    ...
                    // Jangan paksa changeSampleType=1 di setiap submit.
                    // Flag hanya di-set saat user benar-benar ubah jenis/paket/parameter.
                    // Memaksa flag membuat edit nomor sampel saja masuk jalur sync parameter
                    // dan gagal untuk sampel paket kimia+mikro.
                    var changeSampleTypeEl = document.getElementById('changeSampleType');
                    if (changeSampleTypeEl && changeSampleTypeEl.value === '1') {
                        window.__editSampleBootstrapping = false;
                        window.__suppressChangeSampleType = false;
                        // Agar method yang sempat di-disable ikut terserialize saat sync parameter
                        $('.checkbox[name="method[]"]').prop('disabled', false);
                    }

                    function prepareCodeSampleFieldsForSubmit() {
                        var kimiaWrapper = document.getElementById('code_sample_kimia_wrapper_top');
                        var mikroWrapper = document.getElementById('code_sample_mikro_wrapper_top');
                        var kimiaInput = document.getElementById('input_code_sample_kimia');
                        var mikroInput = document.getElementById('input_code_sample_mikro');

                        if (!kimiaInput || !mikroInput) {
                            return;
                        }

                        kimiaInput.disabled = false;
                        mikroInput.disabled = false;

                        var kimiaVisible = kimiaWrapper && kimiaWrapper.offsetParent !== null &&
                            window.getComputedStyle(kimiaWrapper).display !== 'none';
                        var mikroVisible = mikroWrapper && mikroWrapper.offsetParent !== null &&
                            window.getComputedStyle(mikroWrapper).display !== 'none';

                        if (kimiaVisible && !mikroVisible) {
                            mikroInput.disabled = true;
                        } else if (mikroVisible && !kimiaVisible) {
                            kimiaInput.disabled = true;
                        }
                    }
                    ...
                    function suggestReplacementCode(currentCode) {
                        ...
                        var seq = parseInt(String(parts[1]).replace(/\D/g, ''), 10);
                        if (isNaN(seq)) {
                            return currentCode;
                        }
                        parts[1] = String(seq + 1).padStart(4, '0');
                        return parts.join('/');
                    }

                    function submitSampleUpdate(extraData) {
                        var payload = $(form).serializeArray();
                        if (extraData && typeof extraData === 'object') {
                            Object.keys(extraData).forEach(function(key) {
                                payload.push({ name: key, value: extraData[key] });
                            });
                        }
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (response.conflict === true && response.conflicts && response.conflicts.length > 0) {
                                    showSampleCodeConflictDialog(response, form, submitSampleUpdate);
                                    return;
                                }
                    ...
                    function showSampleCodeConflictDialog(response, form, resubmitFn) {
                        var conflict = response.conflicts[0];
                        var wrapper = document.createElement('div');
                        wrapper.style.textAlign = 'left';
                        wrapper.innerHTML =
                            '<p style="margin-bottom:8px;">Nomor <strong>' + escapeHtml(conflict.attempted_code || '-') +
                            '</strong> sudah dipakai sampel lain:</p>' +
                            '<ul style="font-size:13px;padding-left:18px;margin-bottom:12px;">' +
                            '<li>No. saat ini: <strong>' + escapeHtml(conflict.codesample_samples || '-') + '</strong></li>' +
                            '<li>Pelanggan/Titik: ' + escapeHtml(conflict.display_name || conflict.name_customer_pdam || conflict.name_pelanggan || '-') + '</li>' +
                            '<li>Jenis: ' + escapeHtml(conflict.name_sample_type || '-') + '</li>' +
                            '</ul>' +
                            '<label for="swal-conflict-new-code" style="font-size:13px;font-weight:600;">Nomor baru untuk sampel bentrok</label>' +
                            '<input type="text" id="swal-conflict-new-code" class="form-control swal-content__input" ' +
                            'placeholder="Contoh: AM.02/0123/2026" ' +
                            'value="' + escapeHtml(suggestReplacementCode(conflict.codesample_samples || conflict.attempted_code || '')) + '">';

                        swal({
                            title: 'Konflik Nomor Sampel',
                            content: wrapper,
                            icon: 'warning',
                            buttons: {
                                cancel: {
                            ...
                            var newCode = ($('#swal-conflict-new-code').val() || '').trim();
                            if (!newCode) {
                                swal.close();
                                swal('Perhatian', 'Nomor sampel pengganti wajib diisi.', 'warning');
                                $('.btn-simpan').prop("disabled", false);
                                showSampleCodeConflictDialog(response, form, resubmitFn);
                                return;
                            }

                            swal.close();
                            resubmitFn({
                                resolve_sample_code_conflict: '1',
                                conflict_resolutions: JSON.stringify([{
                    ...
                    prepareCodeSampleFieldsForSubmit();
                    submitSampleUpdate();
                }
            });
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            window.moveCheckedParametersToTop = function() {
                ...
                // Scroll to first checked group (only if wizard is initialized)
                if (firstCheckedGroup && window.wizardInitialized) {
                    setTimeout(function() {
                        $('html, body').animate({
            ...
            function updateCartWidget() {
                ...
                // Calculate sampling fee
                const samplingFee = parseInt($('#cost_sampling').val()) || 0;
                const finalTotal = totalPrice + samplingFee;

                // Update price breakdown if packet or sampling fee exists
                let breakdownHTML = '';
                if (packetParams.length > 0 && window.packetPrice) {
                    breakdownHTML += `
                    ...
                    breakdownHTML += `
                        <small class="text-muted d-block mb-1">
                            <i class="fas fa-flask"></i> Total Parameter: ${formatRupiah(totalPrice)}
                        </small>
                    `;
                }
                ...
                if (samplingFee > 0) {
                    breakdownHTML += `
                        <small class="text-muted d-block mb-2">
                            <i class="fas fa-vial"></i> Biaya Sampling: ${formatRupiah(samplingFee)}
                        </small>
                    `;
                }
                ...
                $('#cart-total-price').text(formatRupiah(finalTotal));

                // Sync with form field
                $('#cost_samples').val(totalPrice);
            }

            // Listen for sampling fee changes
            $('#cost_sampling').on('input', function() {
                updateCartWidget();
            });
            ...
            $('#cart-clear-all').on('click', function() {
                ...
                if (confirm('Hapus semua parameter terpilih?')) {
                    $('.checkbox:checked').prop('checked', false).trigger('change');
                    if (typeof markSampleTypeOrParameterChanged === 'function') {
                        markSampleTypeOrParameterChanged();
                    } else {
                        var el = document.getElementById('changeSampleType');
                        if (el) el.value = '1';
