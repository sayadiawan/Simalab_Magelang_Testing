{{--
    Component : Modal Buat Satuan Baru
    Dipakai di: baku-mutu/add.blade.php, baku-mutu/edit.blade.php
    Endpoint  : POST route('elits-units.store') via AJAX
    Fields    : name_unit (wajib), shortname_unit (wajib)
--}}
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" id="btn-simpan-unit">
...
<script>
    (function () {
        ...
        $('#modalCreateUnit').on('hidden.bs.modal', function () {
            var form = document.getElementById('form-create-unit');
            form.reset();
            form.classList.remove('was-validated');
        });

        // Submit via AJAX
        $('#btn-simpan-unit').on('click', function () {
            var form = document.getElementById('form-create-unit');
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        ...
                        var label = response.shortname_unit || response.name_unit || response.id_unit;
                        unitTargets.forEach(function (sel) {
                            var $el = $(sel);
                            if (!$el.length) {
                                return;
                            }
                            if ($el.find('option[value="' + response.id_unit + '"]').length === 0) {
                                $el.append(new Option(label, response.id_unit, false, false));
                            }
                            $el.val(response.id_unit).trigger('change');
                        });
