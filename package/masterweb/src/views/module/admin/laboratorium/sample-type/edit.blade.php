                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ url('/home') }}"><i
                                        ...
                                        Beranda</a></li>
                                <li class="breadcrumb-item"><a href="{{ url('/elits-sampletypes') }}">Sample Type
                                        Management</a></li>
            ...
            <form enctype="multipart/form-data" class="forms-sample" id="form"
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="code_sample_type" name="code_sample_type"
                        value="{{ $sampletype->code_sample_type ?? '' }}" placeholder="Code Sample Type">
                </div>
                ...
                <div class="parameter-section">
                    <div class="parameter-header">
                        ...
                        <div class="action-buttons">
                            <span class="parameter-count" id="method-count">
                                <i class="fas fa-list"></i> {{ count($sampletype_details) }} Parameter
                            </span>
                            <button type="button" class="clear-all-btn" id="clear-method"
                                style="display: {{ count($sampletype_details) > 0 ? 'inline-flex' : 'none' }};">
                    ...
                    <select id="methodAttributes" name="methodAttributes[]" class="form-control" style="display: none;"
                        ...
                        @foreach ($sampletype_details as $sampletype_detail)
                            <option value="{{ $sampletype_detail->id_method }}" selected>
                                {{ $sampletype_detail->params_method }}
                            </option>
                        @endforeach
                    </select>

                    <div class="sortable-container mt-3" id="sortableMethodList">
                        @forelse ($sampletype_details as $index => $sampletype_detail)
                            <div class="sortable-item d-flex justify-content-between align-items-center"
                                data-id="{{ $sampletype_detail->id_method }}">
                                <div class="d-flex align-items-center flex-grow-1">
                                    <span class="drag-handle mr-2">
                                        <i class="fa fa-grip-vertical"></i>
                                    </span>
                                    <div class="order-badge mr-2">{{ $index + 1 }}</div>
                                    <span>{{ $sampletype_detail->params_method }}</span>
                                </div>
                                <span class="remove-item"
                                    onclick="removeItem(this, 'sortableMethodList', '{{ $sampletype_detail->id_method }}')">
                ...
                <div class="parameter-section">
                    <div class="parameter-header">
                        ...
                        <div class="action-buttons">
                            <span class="parameter-count" id="method-plus-count">
                                <i class="fas fa-list"></i> {{ count($sampletype_plus_details) }} Parameter
                            </span>
                            <button type="button" class="clear-all-btn" id="clear-method-plus"
                                style="display: {{ count($sampletype_plus_details) > 0 ? 'inline-flex' : 'none' }};">
                                <i class="fas fa-trash"></i> Hapus Semua
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="sortable-container mt-3" id="sortableMethodPlusList">
                        @forelse ($sampletype_plus_details as $index => $sampletype_plus_detail)
                            <div class="sortable-item d-flex justify-content-between align-items-center"
                                data-id="{{ $sampletype_plus_detail->id_method }}">
                                <div class="d-flex align-items-center flex-grow-1">
                                    <span class="drag-handle mr-2">
                                        <i class="fa fa-grip-vertical"></i>
                                    </span>
                                    <div class="order-badge mr-2">{{ $index + 1 }}</div>
                                    <span>{{ $sampletype_plus_detail->params_method }}</span>
                                </div>
                                <span class="remove-item"
                                    onclick="removeItem(this, 'sortableMethodPlusList', '{{ $sampletype_plus_detail->id_method }}')">
                                    <i class="fa fa-times"></i>
                                </span>
                            </div>
                        @empty
