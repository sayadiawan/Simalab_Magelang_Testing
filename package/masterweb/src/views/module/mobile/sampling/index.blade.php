...
<head>
    ...
    <title>Scan QR Code - Sampling</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="card">
            ...
            @else
                <p>Silakan scan QR code atau masukkan ID permohonan uji untuk memulai proses sampling.</p>
            @endif

            <form method="POST" action="{{ route('mobile.sampling.inputId') }}" style="margin-top: 16px;">
                @csrf
                <label for="id_permohonan" style="display:block;margin-bottom:8px;font-weight:600;text-align:left;">ID
                    Permohonan Uji</label>
                <input id="id_permohonan" name="id_permohonan" class="form-control" placeholder="Masukkan ID permohonan"
    ...
    <script>
        ...
        // Mulai scanner
        document.getElementById("start-scanner").onclick = async () => {
            ...
            if (location.protocol !== "https:" &&
                ...
                alert("Scanner membutuhkan HTTPS.");
                return;
            }
        ...
        function handleScanResult(decodedText) {
            // Extract ID from URL (format: /mobile/sampling/scan/{id})
            ...
            if (id_permohonan) {
                window.location.href = '{{ url('/mobile/sampling/scan') }}/' + id_permohonan;
            }
        }
        ...
        window.onbeforeunload = stopScanner;
    </script>
