...
<head>
    ...
    <title>Input / Output Hasil Px</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="card">
            <h3 style="margin-bottom: 15px; color: #333;">Form Input / Output Hasil Px</h3>
            ...
            <form id="input-hasil-form" method="POST"
                action="{{ route('mobile.testing.storeInputHasil', $sample->id_samples) }}">
                @csrf
            ...
            @if ($existing_verification)
                <a href="{{ route('mobile.testing.bacaHasil', [
                    'id' => $sample->id_samples,
                    'lab_id' => $laboratorium->id_laboratorium,
                    'method_id' => 'bfecda4a-73f2-47d6-9fc3-01f65e0f02a1',
                ]) }}"
                    class="btn btn-primary" style="text-decoration: none; display: block;">
                    <span>📝</span>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            @else
                // Default: Start from Pemeriksaan Stop (same as verification-2.blade.php)
                @if ($pemeriksaan_verif && $pemeriksaan_verif->stop_date)
                    var pemeriksaanStop = new Date("{{ $pemeriksaan_verif->stop_date }}");
                @else
                    var pemeriksaanStop = new Date();
                @endif
            ...
            $('#input-hasil-form').on('submit', function(e) {
                e.preventDefault();
