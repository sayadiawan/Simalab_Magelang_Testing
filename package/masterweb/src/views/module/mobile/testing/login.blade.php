...
<head>
    ...
    <title>Login Petugas Pengujian</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
        ...
        <div class="card">
            <div class="sample-info">
                ...
                <div class="info-item">
                    ...
                    <span><strong>{!! \Smt\Masterweb\Models\Sample::codesampleTableCellHtmlFrom($sample) !!}</strong></span>
                </div>
                <div class="info-item">
                    <label>Jenis Sample:</label>
                    <span>{{ $sample->sampletype->name_sample_type ?? '-' }}</span>
                </div>
                <div class="info-item">
                    <label>Pelanggan:</label>
                    <span>{{ $sample->namaPelangganDisplay() ?? '-' }}</span>
                </div>
                <div class="info-item">
                    <label>Laboratorium:</label>
                    <span>{{ $sample->samplemethod->first()->laboratorium->nama_laboratorium ?? '-' }}</span>
                </div>
            </div>
            ...
            <form action="{{ route('mobile.testing.doLogin', $sample->id_samples) }}" method="POST" autocomplete="off"
                novalidate id="mobileTestingLoginForm">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <div style="margin-top: 8px;">
                        <label
                            ...
                            <input type="checkbox" id="showPasswordMobileTesting"
                                onchange="togglePasswordMobileTesting()" style="margin-right: 6px; cursor: pointer;">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;">
                        <img src="{{ route('captcha.generate') }}?t={{ microtime(true) }}" alt="CAPTCHA"
                            id="captchaImgMobileTesting"
                            style="height:64px; border-radius:8px; background:#eef2ff; padding:4px; cursor:pointer;"
                            title="Klik untuk refresh">
                        <button type="button"
                            onclick="document.getElementById('captchaImgMobileTesting').src='{{ route('captcha.generate') }}?t='+(Date.now())"
    ...
    <script>
        ...
        function togglePasswordMobileTesting() {
            var passwordInput = document.getElementById('password');
            var showPasswordCheckbox = document.getElementById('showPasswordMobileTesting');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var form = document.getElementById('mobileTestingLoginForm');
            if (form) {
                setTimeout(function() {
