...
<head>
    ...
    <title>Pemeriksaan / Analitik</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="card">
            <h3 style="margin-bottom: 15px; color: #333;">Form Pemeriksaan / Analitik</h3>
            ...
            <form id="verification-form" method="POST"
                action="{{ route('mobile.testing.storePemeriksaan', $sample->id_samples) }}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="start_date" name="start_date"
                        ...
                        value="{{ $existing_verification ? \Carbon\Carbon::parse($existing_verification->start_date)->format('d/m/Y') : $default_start_date_str ?? '' }}"
                        required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="stop_date" name="stop_date"
                        ...
                        value="{{ $existing_verification ? \Carbon\Carbon::parse($existing_verification->stop_date)->format('d/m/Y') : $default_stop_date_str ?? '' }}"
                        required>
                </div>
                    ...
                    <select class="form-control" id="nama_petugas" name="nama_petugas" required>
                        ...
                        @foreach ($list_name_petugas as $nama_petugas)
                            <option value="{{ $nama_petugas }}"
                                {{ ($existing_verification && $existing_verification->nama_petugas == $nama_petugas) || $default_analis == $nama_petugas ? 'selected' : '' }}>
                                {{ $nama_petugas }}
                            </option>
                        @endforeach
                    </select>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            let pemeriksaanStart, pemeriksaanStop;

            @if ($existing_verification)
                // Use existing data if available
                pemeriksaanStart = new Date("{{ $existing_verification->start_date }}");
                pemeriksaanStop = new Date("{{ $existing_verification->stop_date }}");
            @else
                // Default: Start from Pendaftaran Stop (same as verification-2.blade.php)
                @if ($pendaftaran_verif && $pendaftaran_verif->stop_date)
                    var pendaftaranStop = new Date("{{ $pendaftaran_verif->stop_date }}");
                @else
                    var pendaftaranStop = new Date();
                @endif
            ...
            @if ($existing_verification)
                start_date_picker.setDate(formatDate(pemeriksaanStart), true);
            @else
            ...
            // Set default date - use existing value if available, otherwise use calculated default
            @if ($existing_verification)
                stop_date_picker.setDate(formatDate(pemeriksaanStop), true);
            @else
                // Use value from input field (which is already set from backend default)
            ...
            $('#verification-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            if (response.next_step === 'baca_hasil' && response.redirect_url) {
                                // Show SweetAlert and redirect to baca-hasil
                                if (typeof Swal !== 'undefined') {
                                    Swal.fire({
                                        title: 'Berhasil!',
                                        text: 'Data verifikasi berhasil disimpan. Lanjut ke Baca Hasil.',
                                        icon: 'success',
                                    ...
                                    if (confirm(
                                            'Data verifikasi berhasil disimpan. Klik OK untuk melanjutkan ke Baca Hasil.'
                                        )) {
                                        window.location.href = response.redirect_url;
                                    }
                                }
