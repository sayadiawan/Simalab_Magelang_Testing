...
<head>
    ...
    <title>Tanda Tangan Nota Kesmas</title>
    <style>
        body {
...
<body>
    <div class="wrap">
        <div class="card">
            <div class="header">
                ...
                <div class="title">Tanda Tangan Nota Kesmas</div>
            </div>
            <div class="muted">Pelanggan: <b>{{ $permohonan->customer->name_customer ?? '-' }}</b></div>
            <div class="muted">ID: <code>{{ $permohonan->id_permohonan_uji }}</code></div>

            <div class="row grid">
                <div>
                    <div class="muted">Tanda Tangan Pelanggan</div>
                    <div class="sig-box"><canvas id="sigCustomer"></canvas></div>
                    <div style="margin-top:8px;display:flex;gap:8px">
                        <button class="btn btn-outline" id="clearCustomer">Bersihkan</button>
                    </div>
                </div>
                <div>
    ...
    <script>
        ...
        const padCustomer = setupPad('sigCustomer');
        const padOfficer = setupPad('sigOfficer');
        document.getElementById('clearCustomer').onclick = () => padCustomer.clear();
        ...
        document.getElementById('saveBtn').onclick = function() {
            ...
            const payload = {
                sig_customer: padCustomer.toDataURL(),
                sig_officer: padOfficer.isEmpty() ? null : padOfficer.toDataURL(),
                _token: '{{ csrf_token() }}'
            };
            fetch('{{ route('mobile.signing.kesmas.nota.save', $permohonan->id_permohonan_uji) }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(r => r.json()).then(j => {
                    if (j.status) {
                        ...
                        window.location.href =
                            '{{ route('mobile.signing.kesmas.nota', $permohonan->id_permohonan_uji) }}';
                    } else {
