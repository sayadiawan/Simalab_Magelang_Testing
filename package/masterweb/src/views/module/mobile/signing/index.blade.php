...
<body>
    <div class="container">
        <div class="header">
            ...
            <h1>Tanda Tangan Nota & Persetujuan</h1>
            <p>Scan QR untuk mendeteksi otomatis: Klinik atau Kesmas</p>
        </div>
        <div class="card">
            @if (session('error'))
            ...
            <h2>Scan QR atau Input ID</h2>
            @if (isset($is_authenticated) && $is_authenticated)
                <div class="alert" style="background:#d4edda;color:#155724;border:1px solid #c3e6cb;">✓ Anda sudah
                    login.</div>
            @endif
            <form method="POST" action="{{ route('mobile.signing.inputId') }}" style="margin-top:16px;">
                @csrf
                <label for="id_permohonan" style="display:block;margin-bottom:8px;font-weight:600;text-align:left;">ID
                    Permohonan</label>
                <input id="id_permohonan" name="id_permohonan" class="form-control" placeholder="Tempel ID dari QR"
                    value="{{ old('id_permohonan') }}" required autocomplete="off">
                <button type="submit" class="btn btn-primary">🔍 Cari & Lanjutkan</button>
            </form>
            ...
            <div id="scanner-section" style="display:none;">
                ...
                <div style="display: flex; gap: 10px; margin-top: 10px;">
                    <button type="button" id="switch-camera" class="btn"
                        style="background:#17a2b8;color:#fff;flex:1;">🔄 Ganti Kamera</button>
                    <button type="button" id="stop-scanner" class="btn"
                        style="background:#dc3545;color:#fff;flex:1;">🛑 Stop Scanner</button>
                </div>
            </div>
    ...
    <script>
        ...
        function handleScanResult(decodedText) {
            // Extract ID from URL (format: /mobile/signing/scan/{id})
            ...
            if (id_permohonan) {
                window.location.href = '{{ url('/mobile/signing/scan') }}/' + id_permohonan;
            }
        }
        ...
        document.getElementById('start-scanner').addEventListener('click', async function() {
            ...
            try {
                ...
                alert('Gagal mengakses kamera. Silakan coba lagi atau gunakan aplikasi QR scanner.');
