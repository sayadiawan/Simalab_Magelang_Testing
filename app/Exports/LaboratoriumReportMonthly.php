...
class LaboratoriumReportMonthly implements FromView, ShouldAutoSize
{
  ...
  public function __construct($month, $year, $program, $sampletype, $laboratorium = null)
  {
  ...
  public function view(): View
  {
    ...
    $data_sample = Sample::whereHas('permohonanuji', function ($query) use ($tahun, $bulan) {
      return $query->whereMonth('date_permohonan_uji', $bulan)
        ->whereYear('date_permohonan_uji', $tahun)
    ...
    if ($laboratorium != null) {
      $data_sample = $data_sample->whereHas('samplemethod', function ($query) use ($laboratorium) {
        return $query->whereNull('tb_sample_method.deleted_at')
    ...
    $method_all = Sample::whereHas('permohonanuji', function ($query) use ($tahun, $bulan) {
      return $query->whereMonth('date_permohonan_uji', $bulan)
        ->whereYear('date_permohonan_uji', $tahun)
        ->whereNull('deleted_at');
    })
    ...
    if ($laboratorium != null) {
      ...
      $method_all = $method_all->join('tb_sample_method', function ($join) {
        $join->on('tb_sample_method.sample_id', '=', 'tb_samples.id_samples')
          ->whereNull('tb_sample_method.deleted_at')
          ->whereNull('tb_samples.deleted_at');
      });
    ...
    $data_sample_methods = Sample::whereHas('permohonanuji', function ($query) use ($tahun, $bulan) {
      return $query->whereMonth('date_permohonan_uji', $bulan)
        ->whereYear('date_permohonan_uji', $tahun)
        ->whereNull('deleted_at');
    })
    ...
    return view('masterweb::module.admin.laboratorium.report.report-monthly.formatPrint.report-monthly-maatweb', [
      ...
      'item_laboratorium' => $item_laboratorium,
