...
class LaboratoriumReportAnually implements FromView, ShouldAutoSize
{
  ...
  public function __construct($year, $program, $sampletype, $laboratorium = null)
  {
    $this->year = (int) $year;
  ...
  public function view(): View
  {
    ...
    $data_sample = Sample::whereHas('permohonanuji', function ($query) use ($tahun) {
      return $query->whereYear('date_permohonan_uji', $tahun)
    ...
    $method_all = Sample::whereHas('permohonanuji', function ($query) use ($tahun) {
      return $query->whereYear('date_permohonan_uji', $tahun)
        ->whereNull('deleted_at');
    })
      ->leftjoin('tb_sample_penanganan', function ($join) {
    ...
    $data_sample_methods = Sample::whereHas('permohonanuji', function ($query) use ($tahun) {
      return $query->whereYear('date_permohonan_uji', $tahun)
        ->whereNull('deleted_at');
    })
      ->orderBy('tb_sample_penanganan.penanganan_sample_date', 'asc')
